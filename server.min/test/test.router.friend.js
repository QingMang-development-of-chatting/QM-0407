...
describe('Friend', function() {
	describe('#getFriends()', function() {
		...
		it('should response 200', async function() {
			this.timeout(50000);
			// case: all undefined
			await friend_requester.getFriends('test_user')
			.then(response => {
	...
	describe('#rejectApplicant()', function() {
		it('should response 400', async function() {
			...
			await friend_requester.rejectApplicant('test_user')
			.then(response => { throw new Error('should not succeed'); })
			.catch(error => {
				expect(error.response.status).to.eql(400);
			});
		...
		it('should response 409', async function() {
			this.timeout(50000);
			// case: all undefined
			await friend_requester.rejectApplicant('test_user', 'applicant1')
			.then(response => { throw new Error('should not succeed'); })
			.catch(error => {
	...
	describe('#getApplicants()', function() {
		...
		it('should response 200', async function() {
			...
			await friend_requester.getApplicants('test_user')
			.then(response => {
				expect(response.status).to.eql(200);
				expect(response.data).to.eql([]);
			});
