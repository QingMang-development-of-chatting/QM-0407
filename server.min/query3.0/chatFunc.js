...
const insertChat = async function(info) {
    if (info.host_id == null || info.room == null)
    {
        return 400;
    }
    else if (info.chat.length != info.date.length)
    {
        console.log(400);
        return 400;
    }
...
const insertRoom = async function(info) {
    if (info.host_id == null || info.user_id == null)
    {
        return 400;
    }
    var result = false;
 ...
 * 输出：
 * --result.num  未读消息数目
 *   result.chat 消息<Array>JSON
 *   result.chat.host_id 发言人
 *   result.chat.room_id
 *   result.chat.chat
 *   result.chat.user_read <Array>
 *   result.chat.date
 * --401 没有已读消息
 */
const readChat = async function(info) {
    if (info.user_id == null || info.room_id == null)
    {
        return 400;
    }
    var result = false;
 ...
 * 输入：
 *   info.room                     
 *   info.param 候选词：key按关键字正则搜索，date输出给定时间的前n条信息，
 ...
 * --info.user_id <Array> (可选)根据精准的user找房间
 * 输出
 * --result.room_id
 *   result.host_id
...
const searchRoom = async function(info) {
    if (info.host_id == null && info.user_id == null && info.room_id == null)
    {
        return 400;
    }
    var result = false;
