...
//mocha -t 5000 test.service.user.js
...
describe('User', function() {
	describe('#register()', function() {
		//username:3~15 不能包含_-之外的特殊字符
		//pw:8~16
		//nickname:1~10
		it('should response REJECT for type error', async function() {
			this.timeout(50000);
			// username too short
			const result_username_too_short = await user_service.register('12', 'test_password', 'test');
			...
			// username too long
			const result_username_too_long = await user_service.register('1234567890123456', 'test_password', 'test');
			...
			const result_username_char = await user_service.register('#123', 'test_password', 'test');
			...
			// password too short
			const result_password_too_short = await user_service.register('123456789012345', '1234567', 'test');
		...
		it('should response OK', async function() {
			...
			const result = await user_service.register('test_user', 'test_password', 'testn');
	...
	describe('#isValid()', function() {
		it('should response NOT FOUND', async function() {
			...
			const result = await user_service.isValid('test_none', 'test_password');
		...
		it('should response REJECT', async function() {
			this.timeout(50000);
			const result = await user_service.isValid('test_user', 'test_error');
			expect(result.status).to.eql(STATUS.REJECT);
		});
	});
	...
	describe('#getInfo()', function() {
		...
		it('should response OK with correct data', async function() {
			...
			expect(result.data).to.eql({username: 'test_user', nickname: 'testn', photo: ''});
		});
	});
	...
	describe('#setPassword()', function() {
		...
		it('should response NOT FOUND', async function() {
			this.timeout(50000);
			// the user called `test_none` should not be created
			const result = await user_service.setPassword('test_none', '123456789');
			expect(result.status).to.eql(STATUS.NOT_FOUND);
		});
	...
	describe('#setNickname()', function() {
		...
		it('should response NOT FOUND', async function() {
			this.timeout(50000);
			// the user called `test_none` should not be created
			const result = await user_service.setNickname('test_none', 'testn_new');
			expect(result.status).to.eql(STATUS.NOT_FOUND);
		});
		...
		it('should response OK and receive correct data', async function() {
			this.timeout(50000);
			// sets new nickname
			const result = await user_service.setNickname('test_user', 'testn_new');
			expect(result.status).to.eql(STATUS.OK);
			// checks if vaild
			const { data: { nickname: nickname } } = await user_service.getInfo('test_user');
			expect(nickname).to.eql('testn_new');
		});
	});
