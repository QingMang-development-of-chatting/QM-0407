...
describe('User', function() {
	describe('#register()', function() {
		...
		it('should response OK', async function() {
			...
			const result = await user_service.register('test_user', 'test_password', 'test');
	...
	describe('#isValid()', function() {
		it('should response NOT FOUND', async function() {
			...
			const result = await user_service.isValid('test_none', 'test_password', 'test');
		...
		it('should response REJECT', async function() {
			this.timeout(50000);
			const result = await user_service.isValid('test_user', 'test_error', 'test');
			expect(result.status).to.eql(STATUS.REJECT);
		});
	});
	...
	describe('#getInfo()', function() {
		...
		it('should response OK with correct data', async function() {
			...
			expect(result.data).to.eql({username: 'test_user', nickname: 'test', photo: ''});
		});
	});
	...
	describe('#setNickname()', function() {
		...
		it('should response NOT FOUND', async function() {
			this.timeout(50000);
			// the user called `test_none` should not be created
			const result = await user_service.setNickname('test_none', 'test_new');
			expect(result.status).to.eql(STATUS.NOT_FOUND);
		});
		...
		it('should response OK and receive correct data', async function() {
			this.timeout(50000);
			// sets new nickname
			const result = await user_service.setNickname('test_user', 'test_new');
			expect(result.status).to.eql(STATUS.OK);
			// checks if vaild
			const { data: { nickname: nickname } } = await user_service.getInfo('test_user');
			expect(nickname).to.eql('test_new');
		});
	});
