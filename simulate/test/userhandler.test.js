...
async function fn() {
	...
	console.log(`Expect ${SOCKET.STATUS.BAD_PARAM}`);
	await new Promise( reslove => socket.emit(USER.LOGIN, 1, 2, result => {
		console.log(result);reslove();
	}));
	await new Promise( reslove => socket.emit(USER.LOGIN, 'Juncheng Zeng', 2, result => {
		console.log(result);reslove();
	}));
	...
	console.log(`Expect ${SOCKET.STATUS.REJECT}`);
	await new Promise( reslove => socket.emit(USER.LOGIN, user.username, user.password, result => {
		console.log(result);reslove();
	}));
	console.log(`Expect ${SOCKET.STATUS.OK}`);
	...
	console.log(`Expect ${SOCKET.STATUS.REJECT}`);
	await new Promise( reslove => socket.emit(USER.LOGIN, user.username, user.password+'1', result => {
		console.log(result);reslove();
	}));
	await new Promise( reslove => socket.emit(USER.LOGIN, user.username+'1', user.password, result => {
		console.log(result);reslove();
	}));
	console.log(`Expect ${SOCKET.STATUS.OK}`);
	await new Promise( reslove => socket.emit(USER.LOGIN, user.username, user.password, result => {
